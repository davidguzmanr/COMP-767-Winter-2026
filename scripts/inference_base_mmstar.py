"""
Run inference on the MMStar benchmark using a pre-trained (non-instruction-tuned)
base VLM.
...
template:
  ...
  - Has chat template (Qwen3.5-4B-Base):
        apply_chat_template(user message with letter-only instruction) — no system prompt.

For instruction-tuned models or fine-tuned PEFT checkpoints use
inference_at_checkpoint_mmstar.py.

Examples:
    python scripts/inference_base_mmstar.py \\
        --model_id google/gemma-3-4b-pt \\
        --output results/mmstar/gemma-3-4b-pt-base.jsonl

    python scripts/inference_base_mmstar.py \\
        --model_id Qwen/Qwen3.5-4B-Base \\
        --output results/mmstar/Qwen3.5-4B-Base-base.jsonl

    python scripts/inference_base_mmstar.py \\
        --model_id meta-llama/Llama-3.2-11B-Vision \\
        --output results/mmstar/Llama-3.2-11B-Vision-base.jsonl
"""
import os
import torch
    ...
    for batch_start in pbar:
        ...
        for example in batch:
            ...
            if has_chat_template:
                ...
                text = f"{image_token} {prompt_text}\nAnswer (A/B/C/D):"
            texts.append(text)
